...
import { HomeIcon, ChatBubbleBottomCenterIcon, BellIcon } from "@heroicons/react/24/outline";
...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      fetchMessages(token); // Obtener los mensajes cuando el usuario está autenticado
    }
  }, [navigate]);
  ...
  const fetchMessages = async (token) => {
    try {
      const response = await axios.get("http://localhost:3000/api/messages/user", {
        headers: {
  ...
  // Filtrar los mensajes por el texto de búsqueda
    ...
    <div className="min-h-screen flex bg-gray-200">
      ...
      <div className={`bg-[#2C3E50] text-white w-64 p-4 space-y-4 ${sidebarOpen ? "block" : "hidden"} md:block`}>
        ...
        <nav className="space-y-2">
          ...
          <button className="flex items-center w-full p-2 rounded hover:bg-gray-700" onClick={() => navigate("/notificaciones")}>
            <BellIcon className="w-6 h-6 mr-2" /> Notificaciones
          </button>
        </nav>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-[#34495E] p-4 flex justify-between items-center">
          <h1 className="text-white text-2xl font-semibold">Mensajes</h1>
          <input
            type="text"
        ...
        <div className="p-6">
          {filteredMessages.length > 0 ? (
            filteredMessages.map((message) => (
              <div key={message._id} className="bg-white p-4 rounded-lg shadow-lg mb-4 hover:bg-gray-100">
